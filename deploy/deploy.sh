...
ssh "$SSH_USER@$TARGET_HOST" "sudo install -d -m 0755 '$REMOTE_SYSTEM_CONFIG_DIR' '$REMOTE_QUADLET_DIR' && sudo install -m 0644 '$REMOTE_CONFIG_PATH' '$REMOTE_SYSTEM_CONFIG_DIR/config.yaml' && sudo install -m 0600 '$REMOTE_KEY_PATH' '$REMOTE_SYSTEM_CONFIG_DIR/id_ed25519' && for unit in /tmp/podman-manager.network /tmp/podman-manager-backend.container /tmp/podman-manager-webapp.container; do sudo install -m 0644 \"\$unit\" '$REMOTE_QUADLET_DIR/' && rm -f \"\$unit\"; done && sudo systemctl daemon-reload && sudo systemctl enable --now podman-manager-backend.service podman-manager-webapp.service && sudo systemctl restart podman-manager-backend.service podman-manager-webapp.service"
